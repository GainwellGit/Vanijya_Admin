...
class Empaccess_model extends CI_Model {
	...
	public function save_empaccess($empid,$empname,$empmobile,$empemail,$empgrade,$emplocation,$empbroad,$empjob){
		...
		$data = ['emp_id' => $empid, 'emp_name' => $empname, 'emp_mobile' => $empmobile, 'emp_email' => $empemail, 'emp_grade' => $empgrade, 'emp_location' => $emplocation, 'emp_broad_function' => $empbroad, 'emp_job_function' => $empjob, 'created_by'=>$updated_by];
		$this->db->select('*');
		$this->db->from('emp_app_access');
