...
class Location extends CI_Controller {
		...
		{
			if(isset($_POST['delete'])){
				$location_auto_id=$this->input->post('zone_id');
				$code=$this->input->post('code');
				$locaion_delete= $this->Location_model->delete_zone_coupon($location_auto_id,$code);
            ...
            if ($this->form_validation->run() == true)
            {
					...
					if(!empty($zone_list)){
						foreach($zone_list as $zones){
						   ...
						   $discount_detail_ids[]=$faq_content;
						}
					}
		...
		public function surcharge(){
            ...
            $getlocation = $this->Location_model->getAllPlants();

			$data['locationlist'] = $getlocation;

			$this->load->ftemplate('locationsurchargeList',$data);
		}

		public function addsurcharge(){

			$plant_surcharge = $this->input->post('plant_surcharge');
			...
			//die($region_surcharge);
			$request = array();
			if ( !empty($plant_surcharge) && !empty($plant_code) && !empty($region_code) ) {
				// code...
            	$data_1 = $this->Location_model->save_region_surcharge($plant_surcharge,$plant_code,$plant_name,$city_name,$po_code,$region_code); 
            	$request = array('success'=>true , 'data' => $data_1);     
			}else{
            	$request = array('success'=>false , 'data' => 'Empty values' , 'plant_surcharge'=> $plant_surcharge , 'plant_code'=> $plant_code);
			}
		...
		public function deletesurcharge(){
			...
			//die($region_code);
			$request = array();
			if ( !empty($plant_code) && !empty($region_code) ) {
				// code...
            	$data_1 = $this->Location_model->delete_region_surcharge($plant_code,$region_code);
            	$request = array('success'=>true , 'data' => $data_1);     
			}else{
            	$request = array('success'=>false , 'data' => 'Empty values' , 'plant_code'=> $plant_code, 'region_code' => $region_code);
			}
			$response = json_encode($request);
			header('Content-Type: application/json');
			echo $response;
